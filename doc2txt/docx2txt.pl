...
if (-d _) {
    ...
    check_for_required_file_in_folder("word/document.xml.rels", $ARGV[0]);
...
if (@ARGV == 1) {
	 ...
	 # WTF? Where does output go if dir passed in? I'll put in in dir/out.txt for now.
	 $ARGV[1] = $ARGV[1] . '/out.txt' if ($inpIsDir eq 'y');
	 
	 # ugh, above flunked so this has to happen only if no .txt yet
     $ARGV[1] .= ".txt" if (!($ARGV[1] =~ m!\.txt$! || $ARGV[1] =~ s/\.docx$/\.txt/));
}
...
if ($inpIsDir eq 'y') {
    readFileInto("$ARGV[0]/word/document.xml.rels", $_);
