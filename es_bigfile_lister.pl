...
# See 'ELASTICSEARCH_MAXFILESIZE_KB' in data\intramine_config.txt for the current value (800 KB or so).
...
die ("Please supply max file size KB as arg: files over that size will be listed.") if (!defined($MaxFileSizeKBArg));

my $Extensions = '(p[lm]|pod|txt|md|js|css|cpp?|c|cc|h|go|bat|html?|xml|log|out|qdoc|textile|cs|vb|java|class|py|php)';
...
# intramine_config.txt ELASTICSEARCH_MAXFILESIZE_KB determines maximum file size for indexing, anything larger isn't indexed.
