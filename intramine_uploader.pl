...
$RequestAction{'req|upload'} =
	\&UploadTheFile;    # req=upload $formH->{'filename'}, 'contents', 'directory'
...
sub UploadTheFile {
	...
	if (   defined($formH->{'filename'})
		&& defined($formH->{'contents'})
		&& defined($formH->{'directory'}))
		{
		...
		if (!IsTextExtensionNoPeriod($ext))
			{
			$result =
				"ERROR, |.$ext| not recognized as a source or text file extension, no copy made.";
			return ($result);
			}
...
sub OkToSave {
	...
	if (defined($formH->{'filename'}))
		{
		my $dir =
			(defined($formH->{'directory'}) && $formH->{'directory'} ne '')
			? $formH->{'directory'}
