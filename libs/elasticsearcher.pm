...
sub GetSearchResults {
	my ($self, $rawquery, $remote, $alllowEditing, $useAppForEditing, $extA,
		$allExtensionsSelected, $folderSearchFilterName, $dir, $numHitsR, $numFilesR)
		= @_;
	my $result = '';
	my $e      = $self->{SEARCHER};
	my $rawResults;
	...
	if ($matchExactPhrase)
		...
		if ($numHits)
			{
			$numFiles = FormatHitResults(
				$rawResults,       $query,         $self->{RAWHITS},
				$remote,           $alllowEditing, $useAppForEditing,
				$matchExactPhrase, $extA,          \$result
			);
			}
		}    # quoted, exact phrase
...
sub GetWordHits {
	...
	# 3. $allExtensionsSelected && $dir ne 'ALL': filter on
	# $folderSearchFilterName = >$dir (as array)
	...
	if ($allExtensionsSelected && $dir eq 'ALL')
		...
		$$rawResultsR = $e->search(
			...
			filter_path => [
				...
				'hits.hits._source.path',      'hits.hits._source.displaypath',
				'hits.hits._source.content',   'hits.hits._source.moddate',
				'hits.hits._source.size',      'hits.hits._source.np_full_title',
				'hits.hits.highlight.content', 'hits.hits._source.title',
				'hits.hits.highlight.title',   'hits.hits._source.full_title',
				'hits.hits.highlight.full_title'
			],
			body => {
...
sub FormatHitResults {
	my (
		$rawResults,       $query,         $maxNumHits,
		$remote,           $alllowEditing, $useAppForEditing,
		$matchExactPhrase, $extA,          $resultR
	) = @_;
				...
				if ($hitCounter < $maxNumHits)
					...
					if (!defined($fullPathSeen{$lcpath}))
						...
						if (!defined($excerptArr) || $excerptArr eq '')
							{
							if (defined($hit->{'highlight'}->{'title'})
								&& $hit->{'highlight'}->{'title'} ne '')
								{
...
"<a href='' onclick = \"editOpen('$pathWithSearchItems'); return false;\">&nbsp;&nbsp;<img src='edit1.png' width='17' height='12' /></a>";
						...
						$displayedPath =
							_StringWithBreaks($longestPathSegmentLength, $displayedPath);

						# Apologies for this line. Link, time stamp, file size, score, file path.
						# And excerpt, way out there at the end.
						my $entry =
"<tr class='oneResult'><td><p class='hitsummary'><strong><span class='resultAnchors'>$anchor$editAnchor</span></strong>&nbsp;&nbsp;<span class='resultTime'>$niceTimeStamp</span>&nbsp;&nbsp; <span class='resultSize'>$fileSize</span>&nbsp;&nbsp; Score: <span class='resultScore'>$score</span> &nbsp;&nbsp;<span class='resultPath'>$displayedPath</span></p>\n<div class='excerpt'>$excerpt</div></td></tr>\n";
						$$resultR .= $entry;
...
sub _StringWithBreaks {
	...
	if (length($displayedPath) > $longestPathSegmentLength)
		...
		if ($lastPosition > 0)
			{
			$displayedPath =
				  substr($displayedPath, 0, $lastPosition + 1)
				. $lineBreaker
