...
sub ConvertTextToHTML {
	...
	my $shouldIncludeMathJax = GetPrettyText($context, $filePath, \$contents); # The text, in tables
	EndHtmlFile($filePath, \$contents, $shouldIncludeMathJax);    # inline JavaScript, finish HTML
	my $outPath = $filePath;
...
FINIS
	...
	$$contents_R .= InlineCssForFile('blockquotes.css');
...
sub GetPrettyText {
	...
	# We are in a table from seeing a line that starts with TABLE[_ \t:.-]?
	# until a line with no tabs.
	my $inATable          = 0;
...
"<li class='$contentsClass' im-text-ln='$lineNum'><a href='#$anchorText'>";
	...
	if (!$isGlossaryFile)
		...
		for (my $i = 0 ; $i < @lines ; ++$i)
			...
			AddGlossaryHints(\${lines [$i]}, $context, 0);
			}
		}
	else
		{
	...
	if (IsSpecialIndexFile($filePath, \@lines))
		...
		$textContents =
"<div id='specialScrollTextRightOfContents' style='background-image: url(data:image/jpeg;base64,$oldPaperImage);'><div id='special-index-wrapper'><table>"
			. join("\n", @lines)
			. '</table></div></div>';
...
sub GlossedFootnote {
	...
	Gloss($footnote, $serverAddr, $mainServerPort, \$glossedFootnote, 0, $IMAGES_DIR,
		$COMMON_IMAGES_DIR, $contextDir, undef, undef, $doNotCacheImages, 1, 0);
	...
	for (my $i = 0 ; $i < @footnoteLines ; ++$i)
		{
		AddGlossaryHints(\${footnoteLines [$i]}, $context, 0);
		}
...
sub GlossedPopupForFootnote {
	...
	if (defined($popupFootnotes{$key}))
		...
		# Last arg "2" means we are in a footnote popup.
		Gloss($footnote, $serverAddr, $mainServerPort, \$glossedFootnote, 1, $IMAGES_DIR,
			$COMMON_IMAGES_DIR, $contextDir, undef, undef, 0, 2, 0);
		...
		# Add glossary popups ("secondary" style only).
		for (my $i = 0 ; $i < @footnoteLines ; ++$i)
			{
			AddGlossaryHints(\${footnoteLines [$i]}, $context, 1);
			}
		...
		$gloss =
			" onmouseover=\"loadAndShowImageHint('$foot', this, event, '600px', false, true);\"";
		}
...
#   -- if you put two or more of '-', '+', or '*', eg '--' or '+++', you'll get a 2nd-level entry.
...
sub Underline {
	...
	my $rowID  = 'R' . $lineNum;
	$$lineR =
"<tr id='$rowID'><td n='$lineNum'></td><td class='vam'><img style='display: block;' src=\"data:image/png;base64,$enc64\" width='98%' height='$height' /></td></tr>";
}
		...
		if ($content =~ m!^\s+$!)
			...
			for (my $j = 0 ; $j <= $lastUsableFieldIndex ; ++$j)
				...
				if ($contentFields[$j] =~ m!^\W$!)
					...
					# We'll adjust for up to 6 spaces at the start of cell contents, replacing every
...
sub IsSpecialIndexFile {
	...
	if ($IndexGetsSpecialTreatment)
		...
		if ($filePath =~ m!$SpecialIndexFileName$!i)
			...
			my $numLines = @$lines_A;
			if (   $numLines
				&& $lines_A->[0] =~ m!$ContentTriggerWord!
				&& $numLines <= 100)
				{
				$result = 1;
				}
...
sub MakeSpecialIndexFileLookSpecial {
	...
	if ($numLines)
		...
		$lines_A->[0] =~
s!(<td>)(.*?$ContentTriggerWord.*?)(</td>)!<th align='center'><span id='toc-line'>$2</span><br/>$flourishImage</th>!i;
		}
}
...
m!((\[([^\]]+)]\((https?://[^)]+)\))|(\"([^"]+)\.\w+(#[^"]+)?\")|(\'([^']+)\.\w+(#[^']+)?\')|\.(\w\w?\w?\w?\w?\w?\w?)(\#[\w:]+)?|((https?://([^\s)<\"](?\!ttp:))+)))!g
		...
		if ($badQuotation)
			...
			my $anchorWithNum =
				(!$haveQuotation && !$haveURL && defined($9)) ? $12 : '';    # includes '#'
			...
			my $haveImageExtension =
				$haveTextExtension ? 0 : (!$haveURL && IsImageExtensionNoPeriod($extProper));
			my $haveVideoExtension = (!$haveURL && IsVideoExtensionNoPeriod($extProper));
			my $haveGoodExtension =
				($haveTextExtension || $haveImageExtension || $haveVideoExtension);    # else URL
			...
			if ($haveGoodExtension)
				{
				$haveGoodMatch = CheckForTextOrImageFile(
					$startPos,           $previousRevEndPos, $ext,
			...
			if ($haveGoodMatch)
				...
				$previousRevEndPos = $len - $previousEndPos -
					1;    # Limit backwards extent of 2nd and subsequent searches.
				}
...
sub CheckForTextOrImageFile {
	my (
		$startPos,      $previousRevEndPos, $ext,                $extProper,
		$haveQuotation, $haveTextExtension, $haveVideoExtension, $quoteChar,
		$anchorWithNum, $isGlossaryFile
	) = @_;
	my $linkIsMaybeTooLong = 0;
	...
	if ($haveQuotation)
		...
		if ($verifiedPath ne '')
			...
			if (FileOrDirExistsWide($IMAGES_DIR . $pathToCheck) == 1)
				...
				&& FileOrDirExistsWide($COMMON_IMAGES_DIR . $pathToCheck) == 1)
				{
				$longestSourcePath = $pathToCheck;
	...
	# NOTE not accounting for paths in "" or '' yet.
	...
	my $haveGoodMatch = 0;
	if (
		($longestSourcePath ne '' || $commonDirForImageName ne '')
		&& (   $extProper =~ m!txt|(html?)$!i
			|| $extProper =~ m!(png|jpeg|jpg|gif|webp)$!i
			|| $haveVideoExtension)
		)
		{
...
sub GetLongestGoodPath {
	my ($doingQuotedPath, $checkStdImageDirs, $revStrToSearch, $currentPath,
		$imageNameR, $commonDirForImageNameR)
		= @_;
	...
	while ($currentRevPos >= 0 || $checkToEndOfLine)
		...
		if ($currentRevPos >= 0)
			...
			my $nextRevTerm =
				substr($revStrToSearch, $prevSubRevPos, $currentRevPos - $prevSubRevPos + 1);
...
sub GetTextFileRep {
	my ($haveQuotation, $quoteChar, $extProper, $longestSourcePath, $anchorWithNum, $repStringR) =
		@_;
...
sub GetVideoRep {
	my ($haveQuotation, $quoteChar, $usingCommonImageLocation, $longestSourcePath,
...
sub GetImageFileRep {
	...
	if (($INLINE_IMAGES || $isGlossaryFile) && !($bestVerifiedPath =~ m!\.gif!i && $hoverGIFS))
		...
		if ($SUPPRESS_IMAGE_LINKS)
			{
			$$repStringR =
"<span href='$bestVerifiedPath' onmouseOver=\"showhint('$bin64Img', this, event, '500px', true);\">$bin64LeftHover$displayedLinkName$bin64RightHover</span>";
			}
		else
			{
			$$repStringR =
"<a href='$bestVerifiedPath' target='_blank' onmouseOver=\"showhint('$bin64Img', this, event, '500px', true);\">$bin64LeftHover$displayedLinkName$bin64RightHover</a>";
			}
		}
}
...
sub PackUpUrlRep {
	...
	# Trim any trailing punctuation character from $url - period, comma etc.
	# Typically those are not part of a URL,
	# eg "if you go to http://somewhere.com/things, you will find...".
	# (Only    .^$*+?()[{\|  need escaping in regex outside of a character class, and only
	# ^-]\  inside one).
...
# NOTE you should prefer putting internal header mentions in "double quotes", sincee that's the only
...
sub EvaluateInternalLinkCandidates {
	...
	while ($currentMatchStartPos > 0)
		...
		# Within range, look for separator characters (space period semicolon etc),
		# collect a potential header.
		while ($prevPos > 0)
			{
			...
			if ($currentPos > 0)
				{
				# Collect next term ($idPart), render it into an acceptable string for an anchor
				# and add it to $potentialID.
				my $idPart = substr($line, $prevPos, $currentPos - $prevPos);
				# First clean up the $idPart.
				# (Note stripping HTML elements will also strip trailing </td></td> if
				...
				# Quotes don't help either.
				$idPart =~ s!['"]!!g;
				# Remove unicode symbols from $id, especially the ones inserted by markdown above,
				# to make it easier to type the headers in links.
				# Eg 'server swarm.txt#TODO_List' for header '&#127895;TODO List'.
				$idPart =~ s!\&#\d+;!!g;    # eg &#9755;
...
sub EndHtmlFile {
	my ($filePath, $contents_R, $shouldIncludeMathJax) = @_;
...
<script>
function loadAndShowImageHint(hintContents, obj, e, tipwidth, isAnImage, shouldDecode) {
...
DONEIT
	...
	if ($shouldIncludeMathJax)
		{
		$useMathJaxValue = 'true';
		}
	$topJS =~ s!_USINGMATHJAX_!$useMathJaxValue!;
	...
	$$contents_R .= InlineJavaScriptForFile('horrible.js');
...
sub LoadGlossary {
	...
	# Unconditionally clear the current definition list.
	%Definition = ();
...
# $inPopup means we are doing a footnote reference popup, at present
# only one level of popups is supported for those, of the "secondary" style
# which is done with CSS directly via display: none/inline-block.
sub AddGlossaryHints {
	my ($txtR, $context, $inPopup) = @_;
	...
	if ($inPopup)
		...
		EvaluateGlossaryCandidates($line, $context, \@repStr, \@repLen, \@repStartPos, undef,
			\%DefinitionSeenOnLine);
		...
		for (my $i = 0 ; $i < $numReps ; ++$i)
			...
			if ($repLen[$i] > 0)
				...
				AddSecondaryGlossaryEntries($context, \@repStr, \@repLen, \@repStartPos, $i,
					$termToSkip);
				}
			}
		}
...
sub EvaluateGlossaryCandidates {
	...
	SortGlossaryResultsForOneLine($repStrA, $repLenA, $repStartPosA);
}
...
sub SkipAltTerms {
	...
	# And also
	# <a%20class='glossary%20term-seen'%20href="#" ...
	if (   $line =~ m!^<a class='glossary'.+?;">([^<]+)</a>!
		|| $line =~ m!^<a%20class='glossary'.+?;">([^<]+)</a>!
		|| $line =~ m!^<a class='glossary term-seen'.+?;">([^<]+)</a>!
		|| $line =~ m!^<a%20class='glossary%20term-seen'.+?;">([^<]+)</a>!)
		{
		my $term = lc($1);
...
sub AddSecondaryGlossaryEntries {
	my ($context, $repStrA, $repLenA, $repStartPosA, $i, $termToSkip) = @_;
	...
	# $termToSkip defined means GetReplacementHint does secondary popup replacement
...
sub GetReplacementHint {
	my ($term, $originalText, $definitionAlreadySeen, $context, $doingSecondaryPopup,
		$doingFootnotePopup)
		= @_;
	my $host       = undef;
	my $port       = undef;
...
"<span class=&quot;popup-wrapper&quot;>$originalText<span class=&quot;popup-content&quot;>___GLOSS_GOES_HERE___</span></span>";
				...
				{
				$result =
"<a class='$class' href=\"#\" onmouseOver=\"showhint('$bin64Img', this, event, '500px', true);\">$originalText</a>";
				}
			}
		else
			{
		...
		if ($gloss =~ m!^<p>(.+?[^\\]):!)
			...
			Gloss("**$termShown**: " . $gloss . $altList,
				$host, $port, \$glossed, 1, $IMAGES_DIR, $COMMON_IMAGES_DIR, $context, undef,
				undef, 0,     2,         0);
...
"<span class=&quot;popup-wrapper&quot;>$originalText<span class=&quot;popup-content&quot;>___GLOSS_GOES_HERE___</span></span>";
			...
			{
			$result =
"<a class='$class' href=\"#\" onmouseover=\"loadAndShowImageHint('$gloss', this, event, '600px', false, true);\">$originalText</a>";
			}
		}
