...
sub ParseCashEvents {
	...
	for (my $row = 0; $row < $numRows; ++$row)
		...
		my @fields;
		_GetCashEventFields($cashEvent, \@fields) or
...
sub RunCashEvents {
	...
	$$detailsR .= "<tr class='openingbal'><td>Opening Balance&nbsp;</td><td>$startYYYYMMStr:</td><td>$balanceDollars</td><td>\&nbsp;</td></tr>";
	...
	for (my $m = $startYYYYMM; $m <= $endYYYYMM; ++$m)
		...
		if ($mon != 12)
			...
			$$detailsR .= "<tr class='$cls'><td>Balance</td><td>$ms</td><td>$balanceDollars</td><td>\&nbsp;</td></tr>";
			}
		else
			{
			push @$dateR, $yr;
			...
			$$detailsR .= "<tr class='$cls'><td>Year End</td><td>$yr</td><td>$balanceDollars</td><td>\&nbsp;</td></tr>";
			}
		# else all done.
		}
	...
	$balanceDollars = ($balance > 0) ? '&nbsp;' . Dollars($balance) : Dollars($balance);
	$$detailsR .= "<tr><td>Final Balance</td><td>$endYYYYMMStr:</td><td>$balanceDollars</td><td>\&nbsp;</td></tr>";
	$$detailsR .= "</table>";
	}
...
sub RunCashEventsForMonth {
	...
	foreach my $evtType (sort {$a cmp $b} keys %$eventDetailsH)
		...
		if ($evtType eq 'Income Annual')
			...
			{
			RunAssetPurchaseForMonth($self, $m, $mon, $ms, $balanceR) or
				return;
			}
		}
	}
...
sub RunAnnualIncomeEventForMonth {
	...
	foreach my $idx (keys %{$eventDetailsH->{'Income Annual'}{'Amount'}})
		...
		if ($startM != $endM)
			{
			RecordFailure($self, "ERROR, start and end months disagree for annual income \'$desc\'\n");
			$result = 0;
			last;
			}
		...
		if ($m >= $startYYYYMM && $m <= $endYYYYMM && $mon == $startM)
			...
			if ($isOneShot)
				{
				$$detailsR .= "<tr><td>ANNUAL INC</td><td>$ms</td><td>$amtDollars</td><td><strong>$desc</strong></td></tr>";
				}
			else
				{
				$$detailsR .= "<tr><td>ANNUAL INC</td><td>$ms</td><td>$amtDollars</td><td><em>$desc</em></td></tr>";
				}
			$$balanceR += $amt;
			}
		}
...
sub RunMonthlyIncomeEventForMonth {
	...
	foreach my $idx (keys %{$eventDetailsH->{'Income Monthly'}{'Amount'}})
		...
		if ($m >= $startYYYYMM && $m <= $endYYYYMM)
			...
			if ($isOneShot)
				{
				$$detailsR .= "<tr><td>MONTHLY INC</td><td>$ms</td><td>$amtDollars</td><td><strong>$desc</strong></td></tr>";
				}
			else
				{
				$$detailsR .= "<tr><td>MONTHLY INC</td><td>$ms</td><td>$amtDollars</td><td><em>$desc</em></td></tr>";
				}
			$$balanceR += $amt;
			}
		}
...
sub RunAnnualExpenseEventForMonth {
	...
	foreach my $idx (keys %{$eventDetailsH->{'Expense Annual'}{'Amount'}})
		...
		if ($startM != $endM)
			{
			RecordFailure($self, "ERROR, start and end months disagree for annual expense \'$desc\'\n");
			$result = 0;
			last;
			}
		#Output("INCOME ANNUAL $ms $desc $amt $startYYYYMMStr $endYYYYMMStr\n") if ($TESTING);
		if ($m >= $startYYYYMM && $m <= $endYYYYMM && $mon == $startM)
			...
			if ($isOneShot)
				{
				$$detailsR .= "<tr><td>ANNUAL EXP</td><td>$ms</td><td>$amtDollars</td><td><strong>$desc</strong></td></tr>";
				}
			else
				{
				$$detailsR .= "<tr><td>ANNUAL EXP</td><td>$ms</td><td>$amtDollars</td><td><em>$desc</em></td></tr>";
				}
			$$balanceR -= $amt;
			}
		}
...
sub RunMonthlyExpenseEventForMonth {
	...
	foreach my $idx (keys %{$eventDetailsH->{'Expense Monthly'}{'Amount'}})
		...
		if ($m >= $startYYYYMM && $m <= $endYYYYMM)
			...
			if ($isOneShot)
				{
				$$detailsR .= "<tr><td>MONTHLY EXP</td><td>$ms</td><td>$amtDollars</td><td><strong>$desc</strong></td></tr>";
				}
			else
				{
				$$detailsR .= "<tr><td>MONTHLY EXP</td><td>$ms</td><td>$amtDollars</td><td><em>$desc</em></td></tr>";
				}
			$$balanceR -= $amt;
			}
		}
...
sub RunAssetPurchaseForMonth {
	...
	foreach my $idx (keys %{$eventDetailsH->{'Asset Purchase'}{'Amount'}})
		...
		if ($m == $startYYYYMM)
			{
			my $amtDollars = Dollars(-$amt);
			$$detailsR .= "<tr><td>ASSET PURCH</td><td>$ms</td><td>$amtDollars</td><td><strong>$desc</strong></td></tr>";
			$$balanceR -= $amt;
			}
		}
	return($result);
