package Net::WebSocket::Server_m;
...
sub new {
	...
	while (my ($key, $value) = each %params)
		...
		croak "$class parameter '$key' expected type is " . ref($self->{$key})
			if ref $self->{$key} && ref $value ne ref $self->{$key};
		$self->{$key} = $value;
		}
	...
	foreach my $watchtype (qw(readable writable))
		...
		croak "$class parameter '$key' expects an arrayref containing an even number of elements"
			unless @{$self->{$key}} % 2 == 0;
		my @watch = @{$self->{$key}};
		$self->{$key} = {};
...
sub _watch {
	...
	for (my $i = 0 ; $i < @_ ; $i += 2)
		...
		my ($fh, $cb) = ($_[$i], $_[$i + 1]);
		croak
"watch_$watchtype expects the second value of each pair to be a coderef, but element $i was not"
			unless ref $cb eq 'CODE';
		if ($self->{"watch_$watchtype"}{$fh})
			{
			carp
"watch_$watchtype was given a filehandle at index $i which is already being watched; ignoring!";
			next;
			}
...
sub start {
	...
	if (ref $self->{listen})
		...
		croak
"failed to start websocket server; the TCP server provided via 'listen' is invalid. (is the listening socket is closed? are you trying to reuse a server that has already shut down?)"
		...
		$self->{listen} = IO::Socket::INET->new(
			Listen    => SOMAXCONN,                # WAS 5, changed 3:31 PM Wednesday, December 31, 2025
	...
	while ($self->{listen}->opened)
		...
		my ($ready_read, $ready_write, undef) =
			IO::Select->select($self->{select_readable}, $self->{select_writable}, undef, $timeout);
		foreach my $fh ($ready_read ? @$ready_read : ())
			...
			if ($fh == $self->{listen})
				...
				{
				warn
"filehandle $fh became readable, but no handler took responsibility for it; removing it";
		...
		foreach my $fh ($ready_write ? @$ready_write : ())
			...
			if ($self->{watch_writable}{$fh})
				...
				{
				warn
"filehandle $fh became writable, but no handler took responsibility for it; removing it";
		...
		if ($self->{silence_max})
			...
			if ($silence_nextcheck < $now)
				...
				$_->{conn}->send('ping')
					for grep {$_->{conn}->is_ready && $_->{lastrecv} < $lastcheck}
					values %{$self->{conns}};
...
sub connections {
	grep {$_->is_ready} map {$_->{conn}} values %{$_[0]{conns}};
}
