...
sub IsNewerThan {
	...
	if (defined($ModTimeForFile{$firstFilePath}))
		...
		my ($dev, $ino, $mode, $nlink, $uid, $gid, $rdev, $size,
			$atime, $mtime, $ctime, $blksize, $blocks) = stat $firstFilePath;
		$firstTime = $mtime;
	...
	if (defined($ModTimeForFile{$secondFilePath}))
		...
		{
		my ($dev, $ino, $mode, $nlink, $uid, $gid, $rdev, $size,
			$atime, $mtime, $ctime, $blksize, $blocks) = stat $secondFilePath;
		$secondTime = $mtime;
...
sub ModDatesAreDifferent {
	...
	my $absTimeDiff = ($firstTime >= $secondTime) ? $firstTime - $secondTime: $secondTime - $firstTime;
...
sub FreshPath {
	...
	if (-f $result)
		...
		my $dir = $path;
		$dir =~ s!/[^/]+$!!; # was  s!/$fileName$!!; - that failed on things like "(2)" in the filename
		$fileName =~ m!(\.[^.]+)$!;
		my $ext = $1;
...
sub FreshPathWithShortenedName {
	my ($path, $maxNameLength) = @_; # $maxNameLength >= 8 total, >= 4 minus extension, or no shortening
	...
	if (-f $result)
		{
		my $freshPath = FreshPath($result); # fresh file name proper could be longer due to _NN on end
		...
		if (-f $result || ($originalMaxNameLength >= 8 && $currentLength > $originalMaxNameLength)) # give up keeping the original file name, just use a number - but keep the extension.
			{
...
sub ShortenedFileName {
	my ($name, $maxNameLength) = @_; # $maxNameLength >= 8 total, >= 4 minus extension, or no shortening
	my $result = $name;
	my $currentLength = length($name);
	if ($maxNameLength >= 8 && $currentLength > $maxNameLength)
		...
		if ($maxProperLength >= 4)
			{
			# Special case: file name ends in [_ ]*\d+[_ ]+ and removing that would leave at least 8 chars in the proper name.
			# Ensure however that if any number can be removed that there is still a number left in the name after removal.
			# (In other words when taking out entire numbers don't take out all the numbers.)
			my $madeItShorter = 1;
			while ($properNameLength >= 8 && $properNameLength > $maxProperLength && $fileNameProper =~ m!\D\d+[_ ]*$! && $madeItShorter)
				{
			...
			if ($properNameLength > $maxProperLength)
				...
				# Nuisance, we might have left part of a number on the end, get rid of it if resulting name would
				# still contain a number and not be too short. Getting too hard to do much better than that.
