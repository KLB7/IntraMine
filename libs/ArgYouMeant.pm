...
#	$kWINDOWS = ($^O =~ m!win!i); 
	if ($kWINDOWS)
		{
		# Aug 2012, OpenDialog() fails if have two or more file pickers - reverting to builtin FileSelect();
...
use Tk::BrowseEntry;
use Tk::Wm;
#use Tk::DateEntry; # Dates no longer supported for Strawberry Perl, the need to install fribidi is too silly.
...
sub _CreateHelpButton {
	...
	my $top = $self->{'TOP'};
	my $helpButton = $top->Button(	-text 		=> 'Help',
									-anchor 	=> 'center',
									-bg 		=> '#0CBA0C',
							  		-command	=> [\&_ShowHelpWindow, $self]);
	$helpButton->grid(-row 		=> $gridRow,
					  -column 	=> 0,
					  -sticky 	=> 'e',
					  -padx 	=> 10);
...
sub _PickColor {
	...
	my $newColor = $top->chooseColor(	-initialcolor 	=> $$colorRef,
                 					-title 			=> "Choose color" );
	if (defined($newColor))
		{
		$$colorRef = $newColor;
