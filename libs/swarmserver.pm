...
my %ShortServerNames
	;   # A list of the short names, so we can recogize them and redirect if the name is not for us.
my %ShortServerNamesType;    # 'PERSISTENT', 'BACKGROUND', or ''
my %PageIsPersistent; # $PageIsPersistent{'Cmd'} = 1 means it survives a shutdown so it can continue
                      #   monitoring status - this is mainly for the "Cmd" page
...
sub LoadServerList {
	...
	if (-f $configFilePath)
		...
		for (my $i = 0 ; $i < @lines ; ++$i)
			...
			if (   length($line)
				&& $line !~ m!^\s*(#|$)!
				&& !($monIsLoaded && $line =~ m!intramine_mon\.pl!))
				{
				...
				if ($specialType ne 'BACKGROUND' && $specialType ne 'WEBSOCKET')
					...
					if ($specialType eq 'PERSISTENT')
						...
						$PageIsPersistent{$pageName} = 1
							; # eg "Cmd" server, will ignore regular 'EXITEXITEXIT' requests so it can monitor during restart
...
sub MakeTopNavTemplate {
	...
	for (my $pgIdx = 0 ; $pgIdx < @PageNames ; ++$pgIdx)
		...
		if ($pageName =~ m!todo!i)
			...
			$entry =
"<li$pageName$hideShowClass><a href='_RESTRICTED_$pageName'>$pageName$overdue</a></li>\n";
			}
		else
			{
			$entry =
				"<li$pageName$hideShowClass><a href='_RESTRICTED_$pageName'>$pageName</a></li>\n";
			}
		$theTopNav .= $entry;
		}
	if (!$forRefresh)
		{
		$theTopNav .=
"<div id='spinnerParent'><img id='spinner' src='globe.gif' width='30.0' height='24.0' /></div>\n";
		$theTopNav .= "</ul>\n";
		...
		$theTopNav .=
			...
			. "<a href='' onclick='showHelpContents(); return(false);'>"
...
# If $timeout is undef (not supplied as an argument), we will loop here forever. If there is a
# timeout supplied, MainLoop will call $DoPeriodic() after the timeout if there are no pending
# requests, and then keep going, also forever.
...
sub MainLoop {
	...
	# Trim trailing slash from $CSS_DIR, often an incoming $obj in ResultPage() has a leading slash.
	$CSS_DIR =~ s!/$!!;
	$JS_DIR = FullDirectoryPath('JS_DIR');
	# Trim trailing slash from $JS_DIR, often an incoming $obj in ResultPage() has a leading slash.
	$JS_DIR =~ s!/$!!;
	$FONT_DIR = FullDirectoryPath('FONT_DIR');
	$FONT_DIR =~ s!/$!!;
	$HELP_DIR = FullDirectoryPath('HELP_DIR');
	$JSEDITOR_DIR =
		FullDirectoryPath('JSEDITOR_DIR');  # for the tinymce or ace or codemirror or whatnot editor
	$HIGHLIGHT_DIR = FullDirectoryPath('HIGHLIGHT_DIR');    # for highlight.js
	$DEFAULT_UPLOAD_DIR =
		FullDirectoryPath('DEFAULT_UPLOAD_DIR');            # for file uploads to the server box
	$DefaultDir               = '';
	$PreviousRefererDir       = '';
		...
		foreach my $sock (@$ready)
			...
			if ($sock == $listener)
				...
				while ($buff = <$sock>)
					...
					# The command server ignores EXIT requests - it stays around to monitor main
					# server if the main server sends an EXITEXITEXIT request here while stopping.
					...
					# HOWEVER, all servers obey a 'FORCEEXIT'.
						...
						{
						# Content-Type:
						# multipart/form-data; boundary=----WebKitFormBoundaryFPkwxXjlBimbXHzJ
				...
				if (!$closed)
					{
					RespondToUnexpectedClose($readable, \$closed, $sock, $InputLinesH, $posted,
						$multiPartFormPostBoundary);
					}
				}
			}
...
sub GrabArguments {
	...
	if (@parts == 3)
		...
		if ($formH->{METHOD} =~ m!post!i)
			{
			# Have your cake and eat it too, as the saying goes. We also check for "get" style
			# arguments, if it's a "Content-Type: multipart/form-data;" post.
			if ($multiPartFormPostBoundary ne '')
				...
				if (!GetPostedFileArguments(\$posted, $multiPartFormPostBoundary, $formH))
					{
					Output(
						"ERROR could not decipher multipart/form-data form data post |$posted|!\n");
					}
				}
			else
				{
...
sub GetPostedFileArguments {
	...
	if ($numPostLines >= 8 && $postLines[0] =~ m!^--$multiPartFormPostBoundary!)
		...
		for (; $i < $numPostLines ; ++$i)
			{
			if ($postLines[$i] =~
m!^Content-Disposition:\s+form-data;\s+name=['"]([^'"]+)['"];\s+filename=['"]([^'"]+)['"]!i
				)
				{
				my $keyName  = $1;
				my $fileName = $2;
		...
		# Pick up name of the "directory" field,
		# from Content-Disposition: form-data; name="directory"
...
sub ResultPage {
	...
	# any short name in the request. If they differ, punt to intramine_main.pl and hope for a
	# response that includes a proper redirect.
	...
	if ($obj =~ m!^/(\w+)(/|$)!)
		...
		if ($ourName ne $incomingName && IsShortServerName($incomingName))
			...
			{
			HandleDefaultPageAction($obj, $formH, $peeraddress, $ourName, \$requestHandled,
				\$result);
			$$reportItR = $ourName;
			}
		}
	...
	# Strip our short name from start of $obj before continuing,
	# leaving one hopes something like a file path.
...
sub GetImageResult {
	...
	if (!$gotIt)
		{
		$gotIt = LoadFileFromSpecificDirectory($JSEDITOR_DIR, $obj, 1, $formH->{REFERER_HREF},
			$mimeTypeR, $resultR);
		}
	if (!$gotIt)
		{
		$gotIt = LoadFileFromSpecificDirectory($HIGHLIGHT_DIR, $obj, 1, $formH->{REFERER_HREF},
			$mimeTypeR, $resultR);
		}
}
...
sub GetCssResult {
	...
	if (!$gotIt)
		{
		$gotIt = LoadFileFromSpecificDirectory($JSEDITOR_DIR, $obj, 0, $formH->{REFERER_HREF},
			$mimeTypeR, $resultR);
		}
	if (!$gotIt)
		{
		$gotIt = LoadFileFromSpecificDirectory($HIGHLIGHT_DIR, $obj, 0, $formH->{REFERER_HREF},
			$mimeTypeR, $resultR);
		}
}
	...
	if ($QUICKDRIVELIST)
		...
		for (my $i = 0 ; $i < @drives ; ++$i)
			...
			if (defined($driveInfo[$i][5]) && $driveInfo[$i][5] ne '')
				...
				my $selected  = '';
				my $optionalName =
					(defined($driveInfo[$i][0]) && $driveInfo[$i][0] ne '')
					? " ($driveInfo[$i][0])"
					: '';
				if ($numDrives == 0)
					{
...
sub LaunchTestProgram {
	...
	if ($testProgramPath ne '')
		...
		my $running = RunPerlProgram($testProgramPath, $serverAddress, $mainPort, $shortName,
			$swarmserverPort);
		if (!$running)
			{
...
sub RunPerlProgram {
	...
	my $proc;
	my $isRunning = 1;
	Win32::Process::Create($proc, $ENV{COMSPEC},
		"/c $path $serverAddress $mainPort $shortName $swarmserverPort",
		0, 0, ".")
		|| ($isRunning = 0);
	return ($isRunning);
}
